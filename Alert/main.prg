...
DEFINE CLASS AlertManager AS Session OLEPUBLIC
	...
	** Full path of the location where the settings file
	** should be written. If VFPAlert.exe is located in a 
	** Vista/Win7 "protected" folder, the settings will 
	** not be saved unless you call SetSettingsPath() 
	** with a writeable location.
	
	** If you create project-specific Desktop Alert EXE's,
	** you could conceivably enter a hard-coded path here.
	cSettingsPath = ""
	
	DIMENSION cSettingsPath_COMATTRIB[5]
	cSettingsPath_COMATTRIB[1] = 0
	cSettingsPath_COMATTRIB[2] = "Full path of a writeable folder for settings file"
	cSettingsPath_COMATTRIB[3] = "cSettingsPath"
	cSettingsPath_COMATTRIB[4] = "String"
	cSettingsPath_COMATTRIB[5] = 0	

	** What sound do we want to play?
	cSound = ADDBS(JUSTPATH(_VFP.ServerName)) + "alert.wav"
	
	DIMENSION cSound_COMATTRIB[5]
	cSound_COMATTRIB[1] = COMATTRIB_HIDDEN
	cSound_COMATTRIB[2] = "Sound file to play when the alert appears (include full path)"
	cSound_COMATTRIB[3] = "cSound"
	cSound_COMATTRIB[4] = "String"
	cSound_COMATTRIB[5] = 0	
	
	** Do we want to reduce memory consumption?
	lCleanMemory = .F.
	
	DIMENSION lCleanMemory_COMATTRIB[5]
	lCleanMemory_COMATTRIB[1] = COMATTRIB_WRITEONLY
	lCleanMemory_COMATTRIB[2] = "True if you want to run DoMemStuff() to reduce memory consumption"
	lCleanMemory_COMATTRIB[3] = "lCleanMemory"
	lCleanMemory_COMATTRIB[4] = "Boolean"
	lCleanMemory_COMATTRIB[5] = 0	
	
	PROCEDURE Init
		This.Setup()
		
		IF This.lCleanMemory
			DoMemStuff()
		ENDIF	
	ENDPROC
	...
	************************************************************
	*  PROCEDURE SetSettingsPath()
	************************************************************
	** Author............: Kevin Ragsdale                     
	** Project...........: VFPAlert                           
	** Description.......: Call this method with the full path
	**                     to the "writeable" folder where the
	**                     alert manager should save the user's
	**                     settings file. 
	** Parameters........: tcPath (folder must already exist!)
	************************************************************
	PROCEDURE SetSettingsPath (tcPath As String) As VOID
		IF DIRECTORY(tcPath,1)
			This.cSettingsPath = ADDBS(tcPath)
		ENDIF
	...
	************************************************************
	PROTECTED PROCEDURE ReadSettings
		...
		IF !EMPTY(This.cSettingsPath)
			IF DIRECTORY(This.cSettingsPath,1)
				lcPath = ADDBS(This.cSettingsPath)
			ELSE
	...
	************************************************************
	PROCEDURE WriteSettings AS VOID
		...
		TEXT TO m.lcSettings TEXTMERGE NOSHOW
		<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
		<DACONFIG>
			<WAIT><<TRANSFORM(This.nWait)>></WAIT>
			<PERCENT><<TRANSFORM(This.nPercent)>></PERCENT>
			<PLAY><<IIF(This.lSound,"1","0")>></PLAY>
		</DACONFIG>
		ENDTEXT
		...
		STRTOFILE(m.lcSettings,lcPath + DA_CONFIGFILE,0)
	ENDPROC
...
DEFINE CLASS Alert AS Session 
	...
	************************************************************
	PROCEDURE Init (toAlertMgr AS Object, tcName AS String)
		SET CLASSLIB TO deskalert.vcx ADDITIVE
		WITH THIS
			.oMgr = toAlertMgr
			.Name = tcName
			.cSound = .oMgr.cSound
...
FUNCTION DoMemStuff As VOID
	** Stole this code from Sergey's blog.
	** This function decreases the amount of
	** memory being used by the Desktop Alerts.
	** http://www.berezniker.com/content/pages/visual-foxpro/lowering-memory-usage-application
